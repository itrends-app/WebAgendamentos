...
$stmt = $pdo->prepare("select * from tb_agendamentos ag, vw_usuarios us where ag.monitor_id = :tutor and str_to_date(ag.data, '%Y-%m-%d') between str_to_date(:data_inicial, '%Y-%m-%d') and str_to_date(:data_final, '%Y-%m-%d') and us.id = ag.aluno_id");
$stmt->bindValue(":tutor", $_SESSION['tutor_id']);
$stmt->bindValue(":data_inicial", $data_inicial);
$stmt->bindValue(":data_final", $data_final);
$stmt->execute();

while ($conn = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $mpdf->WriteHTML('<tr><td>'.utf8_encode($conn['username']).'</td><td>'.$conn['firstname'].'</td><td>'.utf8_encode($conn['horario']).'</td></tr>');
}
